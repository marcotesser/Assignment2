...
public class CashRegister {
    ...
    if(!(bl.get(bl.size()-1).getLocalTime().isAfter(init) && bl.get(bl.size()-1).getLocalTime().isBefore(end))) return false;
    //System.out.println("huhuhuhu1");
    if(!ul.get(ul.size()-1).isMinorenne()) return false;
    //System.out.println("huhuhuhu2");
